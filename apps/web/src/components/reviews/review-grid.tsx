...
import { CheckCircle2, Download, Eye, Filter, MoreHorizontal, Plus, SlidersHorizontal, Star, X, XCircle } from "lucide-react";
...
export function ReviewGrid({ className }: Props) {
  ...
  const StatusCell = useCallback(({ data }: ICellRendererParams<Review>) => {
    ...
    return (
      <Select className="h-7 min-w-[100px] border-0 bg-transparent text-[11px] shadow-none" value={data.status}
        onChange={(e) => { updateStatus(data.id, e.target.value as ReviewStatus); toast.success(`${data.reviewId} → ${REVIEW_STATUS_LABELS[e.target.value as ReviewStatus]}`); }}
  ...
  const columnDefs = useMemo<ColDef<Review>[]>(() => {
    const cols: ColDef<Review>[] = [
      { headerCheckboxSelection: true, checkboxSelection: true, width: 44, pinned: "left", resizable: false, suppressMovable: true, suppressHeaderMenuButton: true },
    ];
      ...
      cellRenderer: (p: ICellRendererParams<Review>) => p.data ? (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={() => { updateStatus(p.data!.id, "approved"); toast.success("Approved"); }}><CheckCircle2 className="mr-2 h-3.5 w-3.5" /> Approve</DropdownMenuItem>
            <DropdownMenuItem onClick={() => { updateStatus(p.data!.id, "rejected"); toast.success("Rejected"); }} className="text-destructive"><XCircle className="mr-2 h-3.5 w-3.5" /> Reject</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ) : null
    });
    ...
    return cols;
  }, [StarCell, SentimentCell, StatusCell, router, visibleColumns, updateStatus]);
