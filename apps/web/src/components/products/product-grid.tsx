...
function BrandFilterDropdown({
  ...
  const filteredBrands = useMemo(() => {
    ...
    if (!q) return FILTER_BRANDS;
    return FILTER_BRANDS.filter((brand) => brand.toLowerCase().includes(q));
  }, [search]);
...
export function ProductGrid({ onEdit, onView, className }: Props) {
  ...
  const archiveProducts = useCallback((targets: Product[]) => {
    const ids = new Set(targets.map((t) => t.id));
    setRowData((rows) =>
      rows.map((r) => (ids.has(r.id) ? { ...r, status: "archived" as ProductStatus } : r)),
    );
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(400px,90vw)] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-input bg-background p-6 shadow-xl">
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <Button
                size="sm"
                variant="destructive"
                onClick={() => archiveProducts(archiveTargets)}
              >
                Archive
              </Button>
            </div>
