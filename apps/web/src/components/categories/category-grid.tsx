...
export function CategoryGrid({ className, addTrigger = 0 }: Props) {
  ...
  const turnOff = useCallback(
    (cat: Category) => {
      patchCategory(cat.id, { active: false });
  ...
  const toggleField = useCallback(
    (cat: Category, field: "active" | "showInTopMenu") => {
      patchCategory(cat.id, { [field]: !cat[field] });
    },
    [patchCategory],
  );

  const bulkUpdate = useCallback(
    (targets: Category[], patch: Partial<Category>, label: string) => {
  ...
  const confirmDelete = useCallback(() => {
    deleteCategories(deleteTargets.map((t) => t.id));
    toast.success(
    ...
    setDeleteTargets([]);
    setSelected([]);
  }, [deleteTargets, deleteCategories]);

  const onCellValueChanged = useCallback(
    (e: { data: Category; colDef: { field?: string } }) => {
      ...
      patchCategory(e.data.id, { [field]: e.data[field as keyof Category] } as Partial<Category>);
  ...
  const onRowDragEnd = useCallback(
    (e: RowDragEndEvent<Category>) => {
      ...
      reorderSiblings(dragged.parentId ?? null, orderedIds);
      toast.success("Order updated");
    },
    [categories, filtered, reorderSiblings],
  );
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-2">
        ...
        <Button
          ...
          onClick={() => {
            ...
            toast.success("Reset to default category order");
          }}
        >
          Reset order
        </Button>
