...
export function BrandGrid({ className, addTrigger = 0, onView, onEdit: onEditProp }: Props) {
  ...
  const getDisplayOrder = useBrandStore((s) => s.getDisplayOrder);
  ...
  const ordered = useMemo(() => getDisplayOrder(), [brands, getDisplayOrder]);
  ...
  const turnOff = useCallback(
    (brand: Brand) => {
      patchBrand(brand.id, { active: false });
  ...
  const toggleActive = useCallback(
    (brand: Brand) => {
      patchBrand(brand.id, { active: !brand.active });
    },
    [patchBrand],
  );

  const bulkUpdate = useCallback(
    (targets: Brand[], patch: Partial<Brand>, label: string) => {
  ...
  const confirmDelete = useCallback(() => {
    deleteBrands(deleteTargets.map((t) => t.id));
    toast.success(`Deleted ${deleteTargets.length} brand${deleteTargets.length > 1 ? "s" : ""}`);
    setDeleteOpen(false);
    setDeleteTargets([]);
    setSelected([]);
  }, [deleteTargets, deleteBrands]);

  const onCellValueChanged = useCallback(
    (e: { data: Brand; colDef: { field?: string } }) => {
      ...
      patchBrand(e.data.id, { [field]: e.data[field as keyof Brand] } as Partial<Brand>);
    ...
    <div className={cn("flex min-h-0 flex-col gap-3", className)}>
      <div className="flex shrink-0 flex-wrap items-center gap-2">
        ...
        <Button
          ...
          size="sm"
          onClick={() => {
            setBrands(brandsSeed);
            toast.success("Reset to default brand order");
          }}
        >
          Reset order
        </Button>
