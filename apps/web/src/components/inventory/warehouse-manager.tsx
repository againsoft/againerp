...
import { Building2, Grid3x3, Package, Plus, Warehouse } from "lucide-react";
...
function WarehouseCard({ wh }: { wh: typeof warehousesSeed[0] }) {
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          <div className="h-9 w-9 rounded-lg bg-indigo-100 dark:bg-indigo-950/40 flex items-center justify-center">
            <Warehouse className="h-4.5 w-4.5 text-indigo-600" />
          </div>
          <div>
...
export function WarehouseManager() {
  ...
  const totalUnits = warehousesSeed.reduce((s, w) => s + w.totalUnits, 0);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Warehouses", value: warehousesSeed.length },
