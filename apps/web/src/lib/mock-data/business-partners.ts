...
export function partnerDirectoryUrlForSupplier(
  supplierId: string,
  tab?: "catalog" | "terms" | "roles",
): string {
  const partner = getPartnerBySupplierId(supplierId);
  const viewId = partner?.id ?? partnerIdForSupplierId(supplierId);
  const params = new URLSearchParams({ view: viewId });
  if (tab) params.set("tab", tab);
  return `/partners/directory?${params.toString()}`;
}

export const VENDOR_DIRECTORY_HREF = "/partners/directory?role=vendor";
