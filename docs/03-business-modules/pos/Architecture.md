...
## Purpose
POS module architecture — scope, features, data ownership, and integration boundaries.

## When To Read
Read this file only if working on POS architecture, features, or module boundaries.
...
Per [MASTER_DATABASE_ARCHITECTURE](../../05-development/database/MASTER_DATABASE_ARCHITECTURE.md), `pos_orders` may be deprecated in favor of `commerce_orders` + `pos_order_links`.
