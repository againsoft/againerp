...
def infer_module(path: Path) -> str:
    ...
    if parts[0] == "modules" and len(parts) > 1:
        return parts[1]
    if parts[0] == "ui-prototype":
        return "ui-prototype"
    if parts[0] == "core":
        return "core"
    if parts[0] == "platform":
        return "platform"
    if parts[0] in ("database", "api", "deployment", "qa", "ui-ux", "framework", "roadmap", "workflows"):
        return parts[0]
    return "platform"
...
def scan_pages() -> list[dict]:
    rows = []
    menu_root = DOCS / "modules" / "ecommerce" / "Menus"
    if menu_root.exists():
